...
const DEFAULT_COLUMNS = [
  "userEmail", "userUserName", "planId", "appName",
  "isUserEmailVerified", "hasPaymentProblem", "createdAt",
];
...
const WildDeerPage = () => {
  ...
  const allColumns = useMemo(() => {
    ...
    accounts.forEach(d => Object.keys(d).forEach(k => { if (k !== "_id") keys.add(k); }));
  ...
  const stats = useMemo(() => ({
    ...
    verified: accounts.filter(a => a.isUserEmailVerified === true).length,
    payIssue: accounts.filter(a => a.hasPaymentProblem === true).length,
    plans: new Set(accounts.map(a => a.planId).filter(Boolean)).size,
  }), [accounts]);
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(a => Object.values(a).some(v => v != null && String(v).toLowerCase().includes(q)));
    }
    if (sortKey) {
    ...
    return result;
  }, [accounts, searchQuery, sortKey, sortDir, quickFilters]);
  ...
  const boolColumns = useMemo(() =>
    allColumns.filter(c => accounts.some(a => typeof a[c] === "boolean"))
  , [allColumns, accounts]);
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true);
    try {
      const [accs, plns] = await Promise.all([fetchMongoAccounts(), fetchPlans()]);
      setAccounts(accs); setPlans(plns);
      toast.success(`Loaded ${accs.length} documents`);
    } catch (e) {
  ...
  const switchEnv = async (mode: EnvMode) => {
    ...
    setEnvMode(mode); setEnvModeState(mode); clearApiAuth();
    setAccounts([]); setPlans([]); setEnvStatus("checking"); setPage(0); setEnvSwitchOpen(false);
  ...
  useEffect(() => { setPage(0); }, [searchQuery, sortKey, sortDir, quickFilters]);
  useEffect(() => {
    ...
    <DashboardLayout>
      ...
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: "Documents", value: stats.total, icon: Users, color: "primary" },
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <div className="relative flex-1">
              ...
              <Input ref={searchRef} placeholder="Search across all fields...   ⌘K"
                value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 pr-9 h-10" />
              {searchQuery && (
            ...
            <div className="flex-1 overflow-y-auto -mx-6 px-6" style={{ minHeight: 0 }}>
              <div className="pb-4">
                {(() => {
                  ...
                  return [...priorityKeys, ...rest].map(key => renderEditField(key));
                })()}
              </div>
            </div>
