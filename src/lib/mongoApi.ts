...
export async function fetchMongoAccounts(limit = 100, skip = 0): Promise<{ data: MongoAccount[]; total: number }> {
  const base = getApiBase();
  const r = await secureFetch(`${base}/accounts?limit=${limit}&skip=${skip}`) as { success: boolean; data: MongoAccount[]; total: number; error?: string };
  if (!r.success) throw new Error(r.error || "Failed to fetch");
  return { data: r.data, total: r.total };
}

export async function searchMongoAccounts(query: string): Promise<{ data: MongoAccount[]; total: number }> {
  const base = getApiBase();
  const r = await secureFetch(`${base}/accounts/search?q=${encodeURIComponent(query)}`) as { success: boolean; data: MongoAccount[]; total: number; error?: string };
